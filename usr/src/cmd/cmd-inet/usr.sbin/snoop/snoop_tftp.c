...
static char *tftp_parse_oack(char *, int, struct tftp_options *);
...
interpret_tftp(int flags, void *data, int fraglen)
{
	...
	if (flags & F_SUM) {
		...
		case DATA:
			(void) sprintf(get_sum_line(),
			    "TFTP Data block %d (%d bytes)%s", ntohs(tftp->th_block),
			    blocksize, blocksize < opts.blksize ? " (last block)":"");
			break;
		case ACK:
	...
	if (flags & F_DTAIL) {
		...
		case WRQ:
			name = (char *)&tftp->th_stuff;
			mode = name + (strlen(name) + 1);
			(void) sprintf(get_line(name - dlc_header, strlen(name) + 1),
			    "File name = \"%s\"", name);
			(void) sprintf(get_line(mode - dlc_header, strlen(mode) + 1),
			    "Transfer mode = %s", mode);
			break;

		case DATA:
			(void) sprintf(
			get_line((char *)(uintptr_t)tftp->th_block - dlc_header, 2),
			    "Data block = %d%s", ntohs(tftp->th_block),
			    blocksize < opts.blksize ? " (last block)" : "");
			(void) sprintf(
			    get_line((char *)(uintptr_t)tftp->th_data - dlc_header,
		...
		case ERROR:
			(void) sprintf(
			    get_line((char *)(uintptr_t)tftp->th_code - dlc_header, 2),
...
static char *
tftp_parse_oack(char *buf, int len, struct tftp_options *opts)
{
	...
	while (size > 0 && idx < sizeof (tftp_options)) {
		...
		/* get name */
		if (idx + strlen(buf) + 1 > sizeof (tftp_options))
			break;
		for (i = 0; i < size; i++) {
