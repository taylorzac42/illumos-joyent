...
qede_ioctl_rd_wr_nvram(qede_t *qede, mblk_t *mp)
{
	...
	case QEDE_NVRAM_CMD_READ:
		...
		ecore_mcp_nvm_read(edev, addr, buf, data1->size);
	...
	case QEDE_NVRAM_CMD_PUT_FILE_DATA:
		...
		ecore_mcp_nvm_write(edev, ECORE_PUT_FILE_DATA,
			  addr, tmp_buf, size);
	...
	case QEDE_NVRAM_CMD_SET_SECURE_MODE:
		ecore_mcp_nvm_set_secure_mode(edev, addr);
		break;

	case QEDE_NVRAM_CMD_DEL_FILE:
		ecore_mcp_nvm_del_file(edev, addr);
		break;

	case QEDE_NVRAM_CMD_PUT_FILE_BEGIN:
		ecore_mcp_nvm_put_file_begin(edev, addr);
		break;

	case QEDE_NVRAM_CMD_GET_NVRAM_RESP:
		buf = kmem_zalloc(size, KM_SLEEP);
		ecore_mcp_nvm_resp(edev, buf);
		(void)memcpy(data1->uabc, buf, size);
		kmem_free(buf, size);
		break;
