...
readagain:
	...
	for (dp = (struct dirent64 *)rddir_data;
		...
		error = nfs4_readdir_getvp(dvp, dp->d_name,
		    &vp, &newexi, req, cs,
		    visp != NULL ? visp->vis_exported : 0, &ae);
		if (error == ENOENT) {
			rddir_next_offset = dp->d_off;
			continue;
		}
